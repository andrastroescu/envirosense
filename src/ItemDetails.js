...
const ItemDetails = ({ item, handleNext, handlePrevious, currentIndex, totalItems }) => {
  return (
    <ul style={{ listStyle: 'none', padding: '0' }}>
      <li style={{ marginBottom: '20px', padding: '10px', border: '1px solid #e0e0e0', borderRadius: '8px', backgroundColor: '#fff' }}>
        ...
        <p><FaCogs /> <strong>Component:</strong> <span style={{ color: 'gray', fontWeight: 'bold', textTransform:'uppercase', fontSize: '1.1rem' }}> {item.component} </span></p>
        <p><FaBoxOpen /> <strong>Material:</strong> <span style={{ color: 'gray', fontWeight: 'normal', fontSize: '1.1rem' }}>{item.material} ({item.percentage}%)</span></p>
